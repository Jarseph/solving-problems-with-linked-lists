...
class Editor {
   ...
   */
  insert(char) {
     if (this.cursor) this.text.insert(char, (node) => node.value === this.cursor.value);
    else this.text.insertAtHead(char);
  ...
  delete() {
    if (this.cursor) {
      ...
      this.arrowLeft();
      this.text.remove((node) => node === index);        
      }
    return this;
  }
  ...
  arrowRight() {
    ...
    else if (!this.cursor) this.cursor = this.text.head;
